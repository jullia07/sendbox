...
class BrickBreakerGame:
    def __init__(self, paddle_color, block_colors=None):
        ...
        self.ball_velocity = [3, -3]  # 공의 속도를 설정하는 부분
    ...
    while running:
        ...
        if game_started and not game_over:
            ...
            if opponent_data is None:
                print("Lost connection to opponent. Exiting...")
                break
            
            if opponent_data == 'WIN' or opponent_data == 'LOSE' :
                pass
            else :
            ...
            # 게임 종료 조건 확인
            if not my_game.blocks:
                print("Sending WIN message")
                my_result = "WIN"
                connection.send("LOSE")  # 상대방에게 패배 메시지 전송
                game_over = True
            elif not opponent_game.blocks:
        ...
        if game_over:
            # 상대방의 승리/패배 메시지를 받기 위해 한 번 더 receive 실행
            if my_result is None:
                print("Waiting for result from opponent")
                my_result = connection.receive()  # 여기서 "WIN" 또는 "LOSE" 메시지를 받아 처리함
