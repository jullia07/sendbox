...
class BrickBreakerGame:
    def __init__(self, paddle_color, block_colors=None):
        ...
        self.ball_velocity = [random.choice([-3, 3]), -3]  # 공의 초기 속도 (x축 방향 무작위)
...
def run_game(is_host, server_ip=None):
    ...
    while running:
        ...
        if game_started and not game_over:
            ...
            # 공이 화면에서 사라지면 패배 처리
            if my_game.ball_pos[1] >= SCREEN_HEIGHT:
                ...
                # 게임 종료 상태인지 확인
                if opponent_data in ["WIN", "LOSE"]:
                    print(f"Received game result: {opponent_data}")
                    my_result = "LOSE" if opponent_data == "WIN" else "WIN"
                    game_over = True
                else:
                    try:
                        # 상대방의 패들과 공의 위치를 처리
            ...
            if not my_game.blocks:
                ...
                my_result = "WIN"
                connection.send("WIN")  # 상대방에게 패배 메시지 전송
                game_over = True
            elif not opponent_game.blocks:
                print("Sending LOSE message")
                my_result = "LOSE"
                connection.send("LOSE")  # 상대방에게 승리 메시지 전송
                game_over = True
